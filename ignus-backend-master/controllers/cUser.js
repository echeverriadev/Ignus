...
async function login(req,res){
	if(req.headers.authorization){
		...
		makeResponseOkMessage(res,'E002')
	}
}
...
async function sendCredentials(userExtension, res) {
	try {
		...
		const { firstName, lastName } = userExtension;
		const emailData = { email: username, password: password, fullname: `${firstName} ${lastName}` }
...
async function addEmployee(req, res) {
	try {
		const employee = await sUser.createUserEmployee(req.fields);
